...
import { Archive, ArrowUpRight, Download, FileImage, FilePlus2, FileSearch, FileText, History, ShieldCheck } from 'lucide-react';
...
export function Assets({ assets, navigate, exportAsset, notify }: {
  ...
  return <>
    <PageHeading eyebrow="内容资产库" title="内容资产" description="统一管理文章、视觉、来源版本、人工决定与不可变资产版本。" action={<button className="primary-action" onClick={() => navigate('studio')}><FilePlus2 size={16} />创建内容任务</button>} />
    ...
    {selected && version && <Drawer title={selected.title} label="资产详情" onClose={() => setSelectedAssetId(null)}>
      ...
      <div className="version-list">{[...selected.versions].reverse().map(item => <button key={item.id} className={`version-item ${item.id === version.id ? 'current' : ''}`} onClick={() => setSelectedVersionId(item.id)}><b>{item.label}</b><div><strong>{item.id === selected.currentVersionId ? '当前版本' : '历史版本'}</strong><span>{item.createdAt} · {item.provenance.origin === 'local' ? '本地工作流' : 'Content Ops API'}</span></div></button>)}</div>
      {version.visualAssets.length > 1 && <><h3><FileImage size={15} />关联视觉</h3><div className="drawer-visual-grid">{version.visualAssets.map(asset => <VisualPreview key={asset.id} asset={asset} compact />)}</div></>}
      ...
      <div className="lineage-detail"><div><span>执行来源</span><strong>{version.provenance.origin === 'local' ? '本地工作流' : 'Content Ops API'}</strong></div><div><span>任务编号</span><strong>{version.provenance.runId}</strong></div><div><span>创建人</span><strong>{version.provenance.createdBy}</strong></div><div><span>创建时间</span><strong>{version.provenance.createdAt}</strong></div></div>
