...
export function Review({ run, editing, selectedFindingId, selectFinding, setEditing, editSection, decide, approve, navigate }: {
  ...
  return <>
    ...
    <div className="review-toolbar panel"><div className="review-title"><ProductVisual product={products[0]} /><div><strong>{run.article.title}</strong><span>Aurelia PHK-01 · B2B SEO 采购指南 · {run.status === 'approved' ? 'V2' : 'V1'}</span></div></div><div className="score-pills"><span><small>质量</small><strong>{run.scores.quality}</strong></span><span className="geo"><small>GEO</small><strong>{run.scores.geo}</strong></span><span className={unresolvedCritical ? 'risk' : 'ok'}><small>关键风险</small><strong>{criticalCount}</strong></span></div></div>
    <div className="review-layout">
      <article className="article-panel panel">
        <div className="document-meta"><span><FileText size={14} />文章草稿 · {articleWordCount(run.article).toLocaleString()} 个英文词 · {run.article.sections.length} 个内容模块</span><em>{run.mode === 'demo' ? '确定性演示输出' : 'Live Lab 输出'}</em></div>
      ...
      <aside className="inspection-panel panel">
        <div className="inspection-head"><div><span className="section-kicker">AI 审查结果</span><h2>审查报告</h2></div><span>{open.length} 未处理</span></div>
